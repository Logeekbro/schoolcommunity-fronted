<template>
  <div class="scroll-box">
    ...
    <el-empty v-if="historys.length === 0" description="无浏览记录"> </el-empty>
    <div v-else>
      <el-alert
        title="注意"
        type="warning"
        description="点击删除按钮后会直接删除记录，没有确认弹窗，请谨慎操作"
        show-icon
      >
      </el-alert>
      ...
      <a-list
        size="large"
        v-for="(history, pindex) in historys"
        :key="pindex"
        bordered
      ...
      >
        <div slot="header">
          <span style="font-size: 25px; color: orange">{{
            getDisplayDate(history.targetDate) +
            " (" +
  ...
  methods: {
    ...
    handleScroll() {
      this.$nextTick(() => {
        ...
        if (
          scrollHeight - scrollTop - windowHeight <= 0.5 &&
          new Date().getTime() - this.fetchTime > 400
        ) {
